...
class UploadHandler
{
    ...
    function __construct($options=null) {
        $this->options = array(
            ...
            'upload_dir' => dirname(__FILE__) . '/../' . $_REQUEST["uploaddir"] . '/',
            'upload_url' => dirname($_SERVER['PHP_SELF']).'/../' . $_REQUEST["uploaddir"] . '/',
            'param_name' => 'files',
            ...
            'image_versions' => array(
                ...
                */
                'thumbnail' => array(
                    'upload_dir' => dirname(__FILE__).'/../temp/thumbnails/',
                    'upload_url' => dirname($_SERVER['PHP_SELF']).'/../temp/thumbnails/',
                    'max_width' => 80,
                    'max_height' => 80
                )
            )
    ...
    private function handle_file_upload($uploaded_file, $name, $size, $type, $error) {
        ...
        if (!$error && $file->name) {
            $file->name = str_ireplace(" ", "_", $file->name);
            ...
            if (!file_exists($_SERVER["DOCUMENT_ROOT"] . "/" . $_REQUEST["uploaddir"])
            &&  !is_dir($_SERVER["DOCUMENT_ROOT"] . "/" . $_REQUEST["uploaddir"])) {
              mkdir($_SERVER["DOCUMENT_ROOT"] . "/" . stripouterslashes($_REQUEST["uploaddir"]), 0777, TRUE);
            }
